...
SQUIRREL_TWEETS = [
    "<em class='green'>Gonna finish that bagel? lol</em>",
    "<em class='red'>Squirrels <3 SXSW lol</em>",
    "<em class='yellow'>Keep Austin squirrelly lol</em>"
]
...
class NotifyHandler(webapp2.RequestHandler):
  ...
  def _handle_locations_notification(self, data):
    ...
    text = 'UMSI locator says you are at %s by %s.' % \
  ...
  def _handle_timeline_notification(self, data):
    ...
    for user_action in data.get('userActions', []):
      ...
      if user_action.get('type') == 'SHARE':
        ...
        body = {
            'text': 'Your media has been submitted to UMSI! %s' % item.get('text', '')
        }
        ...
        utterance = choice(SQUIRREL_TWEETS)

        item['text'] = None
        item['html'] = ("<article class='auto-paginate'>" +
            "<p class='text-auto-size'>" +
            "Squirreled away for tweeting: " + note_text + ". " + utterance + "</p>" +
            "<footer><p>UMSI SquirrelModerator</p></footer></article>")
